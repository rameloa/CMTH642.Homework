...
```{r}
wine.train_index <- sample(1:nrow(wine.df), 0.7*nrow(wine.df))
wine.train <- wine.df[wine.train_index,]
wine.test <- wine.df[-wine.train_index,]
(nrow(wine.df) == (nrow(wine.train) + nrow(wine.test))) # Did we include all observations?


```

7. Use the KNN algorithm to p redi ct the quality of wine using its attributes.
```{r}
...
# Changing the value of K didn't make much difference but k=3 had best results
# Otherwise maybe this wasn't a great classifier...
# TP rates were (k=3): 53.1%, 79.6% and 15.9% for Low, medium & high respectively
